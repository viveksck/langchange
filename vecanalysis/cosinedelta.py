...
from googlengram import ioutils
...
CONTEXT_WORDS = context_words = ioutils.load_pickle("/dfs/scratch0/google_ngrams/info/interestingwords.pkl")
WORDS = ioutils.load_pickle("/dfs/scratch0/google_ngrams/info/interestingwords.pkl")
REP_TYPE = "SKIPGRAM"
...
def merge():
    ...
    for word in WORDS:
        ...
        disp_yearstats[word] = {}
    for year in YEARS:
        vol_yearstat = ioutils.load_pickle(TMP_DIR + str(year) + "-vols.pkl")
        disp_yearstat = ioutils.load_pickle(TMP_DIR + str(year) + "-disps.pkl")
        for word in WORDS:
    ...
    ioutils.write_pickle(vol_yearstats, OUTPUT_PREFIX + "-vols.pkl")
    ioutils.write_pickle(disp_yearstats, OUTPUT_PREFIX + "-disps.pkl")
...
def main(proc_num, lock):
    while True:
        ...
        ioutils.write_pickle(year_vols, TMP_DIR + str(year) + "-vols.pkl")
        ioutils.write_pickle(year_disp, TMP_DIR + str(year) + "-disps.pkl")
