...
class PackedVector(object):
    ...
    def __init__(self, *shapes, **shapemap):
        ...
        # Generate endpoints
        self._dims = shapes
        self._lens = map(np.prod, self._dims)
        self._ends = np.concatenate([[0], np.cumsum(self._lens)])
        self._vec = np.zeros(self._ends[-1]) # allocate storage
...
class NNBase(object):
    ...
    def grad_check(self, x, y, eps=1e-4, tol=1e-6,
        ...
        for name in self.params.names():
            ...
            print >> outfd, "    %s dims: %s = %d elem" % (name, str(list(theta.shape)), np.prod(theta.shape))
        ...
        for name in self.sparams.names():
            ...
            for idx, grad_computed in self.sgrads[name]:
                ...
                print >> outfd, ("[ok]" if grad_delta < tol else "**ERROR**")
                print >> outfd, "    %s[%s] dims: %s = %d elem" % (name, idx, str(list(theta.shape)), np.prod(theta.shape))
                if verbose and (grad_delta > tol): # DEBUG
                    print >> outfd, "Numerical: \n" + str(grad_approx)
                    print >> outfd, "Computed:  \n" + str(grad_computed)
    ...
    def randomiter(N, high, batch=1):
        ...
        for i in xrange(N):
            yield np.random.randint(0, high, size=batch)
