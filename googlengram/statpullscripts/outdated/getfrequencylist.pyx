...
OUTPUT_FILE = DATA_DIR + "/info/freqsorted.tsv"
...
def compute_word_stats(freqs, word_i):
    try:
        return freqs[word_i][0]
    except KeyError:
        return 0
...
def main(proc_num, lock):
    ...
    while True:
        ...
        print proc_num, "Retrieving freqs for year", year
        freqs = util.load_pickle(INPUT_DIR + str(year) + "-freqs.pkl")
        word_stats = {}
        print "Getting stats for year.."
        for word_i in COMMON_INDICES: 
            word_stats[MERGED_LIST[word_i]] = compute_word_stats(freqs, word_i)
