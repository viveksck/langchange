...
def make_knn_mat(old_mat, k):
    ...
    for i in xrange(len(old_mat.data)):
        if old_mat.data[i] < row_keeps[old_mat.col[i]] and old_mat.data[i] < row_keeps[old_mat.row[i]]:
            old_mat.data[i] = 0
...
def main(proc_num, lock, in_dir, years, k):
    ...
    while True:
        ...
        temp = coo_matrix((data_d, (row_d, col_d)))
        
        print proc_num, "Writing counts for year", year
        matstore.export_mat_eff(temp.row, temp.col, temp.data, year, tmp_pref)
...
if __name__ == '__main__':
    ...
    ioutils.mkdir(args.in_dir + "/knn-" + str(args.k))
