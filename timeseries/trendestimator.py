...
def get_new_words(word_series, start_year=1900, end_year=2000, not_present_min=0.25, not_present_max=0.75, buffer=10, thresh=0.0):
    ...
    for word, year_series in word_series.iteritems():
        ...
        num_not_pres = 0
        for year in years:
            if not year in year_series or year_series[year] <= thresh or year_series[year] == float('nan'):
                last_not_pres = max(year, last_not_pres)
                num_not_pres += 1
        if float(num_not_pres)/len(years) > not_present_min and float(num_not_pres)/len(years) < not_present_max and last_not_pres <= end_year - buffer and word in inc_words:
            new_words[word] = last_not_pres + 1
    return new_words
